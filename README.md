...
**Three surfaces need no account at all.** They are the load-bearing claim of this project, so they were built to be checked by a stranger:

| Try this | Where | What it proves |
|---|---|---|
| Drop any generated file in | [**/verify**](https://originshot.vercel.app/verify) | Provenance re-derived from the bytes alone. Edit one pixel and it reports `content_bound ✗ BROKEN`. |
| Paste a marketplace listing URL | [**/check**](https://originshot.vercel.app/check) | Buyer-side recognition of a re-encoded copy, after the manifest is stripped. |
| Replay the hash chain yourself | [**/ledger**](https://originshot.vercel.app/ledger) | Every manifest appended to a published, signed, Bitcoin-anchored log. |

**To see the seller side, sign in with the shared demo account** — deliberately public, pre-loaded, and safe to break:

```
https://originshot.vercel.app/signin
  email     judge@originshot.app
  password  verify-the-pixels
```

The sign-in page also carries a one-click **"Explore a pre-loaded demo account"** button, so nothing needs typing.

**Or prove the whole thesis in one command** — no account, no browser:

```bash
curl -s https://originshot-api.onrender.com/api/verify/f021cd558fed75cd80c3c69fb3cd770440d28b5ad0f55b9c38be8aba8534775b
```
...
> **First request may take ~50s.** The API is on Render's free tier and sleeps after ~15 min idle; a [keep-warm workflow](.github/workflows/keep-warm.yml) pings it every 10 min, but a cold start is possible. Everything after the first request is fast.

Full endpoint list, runnable in the browser: [**Swagger UI**](https://originshot-api.onrender.com/docs).
